...
class CacheEngine:
    ...
    def reset_gpu_cache(self):
        for idx in range(len(self.gpu_cache)):
            k, v = self.gpu_cache[idx]
            self.gpu_cache[idx] = (None, None)
            del k
            del v
        del self.gpu_cache
        ...
        logger.info("resetting GPU cache to : {} blocks".format(self.num_gpu_blocks))
        self.gpu_cache = self.allocate_gpu_cache()
