...
class WorkerLoRAManager(AbstractWorkerLoRAManager):
    ...
    def _store_lora(self, lora_request: LoRARequest) -> bool:
        try:
            flat_tensor_rt, tensor_names, tensors_metadata = self._lora_model_cls.store_from_local_checkpoint(lora_request.lora_local_path)
            self.lora_storage[lora_request.lora_int_id] = (flat_tensor_rt, tensor_names, tensors_metadata)
        except Exception as e:
            raise RuntimeError("Storing lora: {} failed".format(lora_request.lora_name)) from e

    def _load_lora(self, lora_request: LoRARequest) -> LoRAModel:
        try:
            if not self.lora_config.use_aqua:
                ...
                flat_tensor_rt, tensor_names, tensors_metadata = self.lora_storage[lora_request.lora_int_id]
                lora = self._lora_model_cls.from_stored_flat_tensor(
                    flat_tensor_rt, 
                    tensor_names, 
                    tensors_metadata,
                    lora_request.lora_local_path,
                    ...
                    embedding_modules=self.embedding_padding_modules,
                    embedding_padding_modules=self.embedding_padding_modules
                )
        except Exception as e:
            raise RuntimeError(
...
class LRUCacheWorkerLoRAManager(WorkerLoRAManager):
    ...
    def create_lora_manager(
        ...
        logger.info("CREATING LRU MANAGER")
        return lora_manager.model
    ...
    def add_lora(self, lora_request: LoRARequest) -> bool:
        if lora_request.lora_int_id not in self.list_loras():
            ...
            if len(self._lora_manager) + 1 > self._lora_manager.capacity:
                logger.info("Removing oldest lora")
                self._lora_manager.remove_oldest_lora()
            logger.info("Loading new lora")
            # Loading keeps it in CPU memory
            lora = self._load_lora(lora_request)
            # Add, adds it to the LRU list
            loaded = self._lora_manager.add_lora(lora)
        else:
            # If the lora is already loaded, just touch it to
            # update its position in the caches
            loaded = self._lora_manager.get_lora(lora_request.lora_int_id)
        # Activate, copies it to the destination
