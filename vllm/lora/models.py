...
class LoRAModel:
    ...
    @classmethod
    def store_from_local_checkpoint(
        cls,
        lora_dir: str
    ) -> Tuple[aqua.responsive_tensor, List[str], Dict[str, Tuple[torch.Size, int]]]:
        """Store a LoRAModel from a local checkpoint."""
        ...
        for tensor_name in tensors:
            ...
            flattened_tensors.append(flat_view)
            # store tensor shape and offset
            tensors_metadata[tensor_name] = (tensor.shape, current_offset)
            tensor_names.append(tensor_name)
            current_offset += flat_view.numel()

        flat_tensor = torch.cat(flattened_tensors, dim=0)
        # Call aqua here
        flat_tensor_rt = aqua.responsive_manager.to_responsive_tensor(flat_tensor)
        logger.info("FLAT TENSOR SIZE: {} MB".format(flat_tensor.numel() * flat_tensor.element_size() / (1024 * 1024)))
        del flat_tensor

        return flat_tensor_rt, tensor_names, tensors_metadata
    
    @classmethod
    def from_stored_flat_tensor(cls, 
        flat_tensor_rt: aqua.responsive_tensor, 
        tensor_names: List[str], 
        tensors_metadata: Dict[str, Tuple[torch.Size, int]],
        lora_dir: str,
        lora_model_id: Optional[int] = None,
        dtype: Optional[torch.dtype] = None,
        target_embedding_padding: Optional[int] = None,
        embedding_modules: Optional[Dict[str, str]] = None,
        embedding_padding_modules: Optional[List[str]] = None) -> "LoRAModel":

        """Create a local LoraModel after copying tensor from the remote memory"""
        
        flat_tensor = flat_tensor_rt.to_torch_tensor()
        logger.info("FLAT TENSOR SIZE: {} MB".format(flat_tensor.numel() * flat_tensor.element_size() / (1024 * 1024)))
        ...
        new_embeddings_bin_file_path = os.path.join(lora_dir,
                                                    "new_embeddings.bin")
        flat_tensor = flat_tensor.to("cuda")
        prev_offset = 0
        for tensor_name in tensor_names:
            shape, offset = tensors_metadata[tensor_name]
            tensor_size = math.prod(shape)
            assert offset == prev_offset
            recreated_tensors[tensor_name] = flat_tensor[offset: offset + tensor_size].view(shape)
            prev_offset = offset + tensor_size
    ...
    def from_local_checkpoint(
        ...
        for tensor_name in tensor_names:
            ...
            recreated_tensors[tensor_name] = flat_tensor[offset: offset + tensor_size].view(shape)
            prev_offset = offset + tensor_size
            assert torch.equal(recreated_tensors[tensor_name], tensors[tensor_name])
